...
class SearchParameters:
    ...
    def from_input(cls) -> 'SearchParameters':
        ...
        while True:
            try:
                initial_top_k = int(input(f"初期検索件数 (デフォルト: {DEFAULT_INITIAL_TOP_K}): ") or DEFAULT_INITIAL_TOP_K)
                if initial_top_k < 1:
                    print("初期検索件数は1以上の値を指定してください。")
                    continue
                    
                final_top_k = int(input(f"最終検索件数 (デフォルト: {DEFAULT_FINAL_TOP_K}): ") or DEFAULT_FINAL_TOP_K)
                if final_top_k < 1 or final_top_k > initial_top_k:
                    print("最終検索件数は1以上、かつ初期検索件数以下の値を指定してください。")
                    continue
                    
                semantic_weight = float(input(f"意味検索の重み (0-1の範囲, デフォルト: {DEFAULT_SEMANTIC_WEIGHT}): ") or DEFAULT_SEMANTIC_WEIGHT)
...
class RerankerModel:
    ...
    def rerank(self, query: str, passages: List[Dict]) -> List[Dict]:
        ...
        for score, passage in scored_passages:
            ...
            passage_with_score['reranking_score'] = float(score)  # Convert torch.Tensor to float if necessary
...
class LoanAssistantBot:
    ...
    def __init__(self, 
        ...
        self.embeddings = self._create_embedding_model(embedding_provider, embedding_model_name)
    ...
    def split_by_hierarchical_headers(self, text: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            is_major_header = any(re.match(pattern, line) for pattern in major_header_patterns)
            is_sub_header = any(re.match(pattern, line) for pattern in sub_header_patterns)
            ...
            if path_text:
                chunk = Document(
                    ...
                    metadata={
                        ...
                        'hierarchy': {k: v for k, v in hierarchy.items() if v is not None}
                    }
                )
    ...
    def _load_single_pdf(self, pdf_path: str) -> None:
        ...
        if isinstance(md_text, list):
            full_text = "\n\n".join(page["text"] if isinstance(page, dict) else str(page) for page in md_text)
        else:
    ...
    def load_documents_from_directory(self) -> None:
        ...
        documents = self._find_documents_recursively(DOCS_DIRECTORY)
        print(f"Found {len(documents['pdf'])} PDF files and {len(documents['excel'])} Excel files")
    ...
    def _load_important_terms(self) -> Set[str]:
        """専門用語をJSONファイルから読み込む"""
        terms_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "terms", "finance_terms.json")
        try:
    ...
    def _extract_keywords(self, text: str, context_window: int = 5) -> List[Tuple[str, float]]:
        ...
        for i, (word, pos) in enumerate(valid_words):
            ...
            context_words = set(w for w, _ in valid_words[start_idx:i] + valid_words[i+1:end_idx])
            
            # 重要語句との共起スコアを計算
            context_importance = sum(1.0 for w in context_words if w in important_terms)
            context_score = (len(context_words) + context_importance) / (2 * context_window)
            
            # 5. 文字長による重み付け（より長い複合語を優先）
            length_weight = min(len(word) / 10.0, 1.5)
            
            # 6. 質問文中の疑問詞に関連する単語の重要度を上げる
            question_weight = 1.1 if re.search(r'(どの|何|いつ|どう|どちら|いかが)', text) else 1.0
        ...
        for chunk in self.chunks:
            ...
            for q_keyword, q_score in question_keywords:
                for c_keyword, c_score in chunk_keywords:
                    if q_keyword.lower() in c_keyword.lower() or c_keyword.lower() in q_keyword.lower():
                        # キーワードの重要度を考慮したスコア
    ...
    def ask(self, question: str) -> str:
        ...
        for source in sorted(referenced_files):
            # DOCSディレクトリの絶対パスを使用
            full_path = os.path.join(DOCS_DIRECTORY, source).replace('/', '\\')  # Windowsパス形式に変換
        ...
        if hasattr(self, 'prompt_template'):
            ...
            raise ValueError("Prompt template not loaded. Please provide a prompt template file path when initializing the bot.")
        ...
        for i, chunk in enumerate(relevant_chunks, 1):
            # DOCSディレクトリの絶対パスを使用
            full_path = os.path.join(DOCS_DIRECTORY, chunk['source']).replace('/', '\\')  # Windowsパス形式に変換
            reference_info += f"\n\n[参照チャンク {i}]\n"
...
def main():
    ...
    try:
        ...
        print(f"\n融資業務Q&Aボットです。ドキュメントは {DOCS_DIRECTORY} から再帰的に読み込まれます。")
        print("質問を入力してください（終了は'quit'）")
        print(f"使用中の埋め込みモデル: {bot.embeddings.model_name}")
        print(f"使用中のリランカーモデル: {RERANKER_MODEL}")
        print("\n現在の検索パラメータ:")
        print(f"- 初期検索件数: {bot.search_params.initial_top_k}")
        print(f"- 最終検索件数: {bot.search_params.final_top_k}")
        print(f"- 意味検索の重み: {bot.search_params.semantic_weight}")
        ...
        while True:
            ...
            if question.lower() == 'quit':
                ...
                # 検索パラメータの再設定
                bot.search_params = SearchParameters.from_input()
                bot.save_search_params()
                print("\n検索パラメータを更新しました:")
                print(f"- 初期検索件数: {bot.search_params.initial_top_k}")
                print(f"- 最終検索件数: {bot.search_params.final_top_k}")
                print(f"- 意味検索の重み: {bot.search_params.semantic_weight}")
