...
// --- FR-007: FAQ削除確認 (Action.Execute verb: confirmDeleteFaqs) ---
agentApp.adaptiveCards.actionExecute(
  "confirmDeleteFaqs",
...
// --- FR-007: FAQ削除実行 (Action.Execute verb: executeDeleteFaqs) ---
agentApp.adaptiveCards.actionExecute(
  "executeDeleteFaqs",
  async (context: TurnContext, _state: TurnState, data: Record<string, unknown>) => {
...
// --- FR-008: シナリオ要修正フラグ保存 (Action.Execute verb: saveNeedsUpdate) ---
agentApp.adaptiveCards.actionExecute(
  "saveNeedsUpdate",
  async (context: TurnContext, _state: TurnState, data: Record<string, unknown>) => {
...
// --- FR-009: Excel出力 (Action.Execute verb: exportExcel) ---
...
async function executeSearch(
  ...
  try {
    ...
    // FR-009: 検索結果をキャッシュ（ページネーション + Excel出力用）
    cleanExpiredCache();
