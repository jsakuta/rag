 ...
 *
 * 使用法:
 *   node scripts/seed-cosmos.js              # JSON から upsert
 *   node scripts/seed-cosmos.js --clean       # 既存データ全削除後に投入
 *
 * 前提:
...
async function deleteAllItems(container, containerName) {
...
async function upsertBatch(container, docs, containerName) {
  let upserted = 0;
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    process.stdout.write(`\r  ${containerName}: ${upserted}/${total}件投入`);
  }
...
async function main() {
  ...
  console.log("\n" + "=".repeat(60));
  console.log(`完了: scenarios ${sCount}件, faqs ${fCount}件を投入しました`);
  console.log("=".repeat(60));
}
