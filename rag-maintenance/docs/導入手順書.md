...
# Cosmos DB Built-in Data Contributor ロールを付与
MSYS_NO_PATHCONV=1 az cosmosdb sql role assignment create \
  --account-name cosmos-maintenance-poc \
  --resource-group rg-maintenance-poc \
  --role-definition-id "00000000-0000-0000-0000-000000000002" \
  --principal-id "<上記で取得したオブジェクトID>" \
  --scope "/"
```
...
| `Invalid URL`でサーバーが起動しない | `.localConfigs`の`AI_SEARCH_ENDPOINT`等が空 | `.localConfigs`の必須項目を確認・設定（上記15.1参照） |
| F5デバッグで認証エラー | `.localConfigs`の`clientId`/`clientSecret`が空 | `env/.env.dev`からBOT_IDを転記、シークレットは`az ad app credential reset`で再生成（上記15.1参照） |
