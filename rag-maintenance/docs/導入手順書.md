...
全14ステップを順番に実行することで、Teams Bot による影響候補検出システムが稼働する。
...
> **注意:** Microsoft 365 へのサインインはこの時点では不要。Provision実行時（Step 7「Provision実行」）に求められるため、そのタイミングで行う。
...
> **補足:** テンプレート選択時の全体構造は以下の通り。本手順ではBotアプリからAzure OpenAIを直接呼び出さない方針（要件定義書8.4参照）だが、M365 Agents SDKベースのプロジェクト構造を得るためCustom Engine Agentを選択する。LLM接続部分はStep 7「m365agents.yml のカスタマイズ」で削除・変更する。
>
...
## Step 10: Managed Identity ロール付与
...
**Built-in Data ContributorのロールID:** `00000000-0000-0000-0000-000000000002`（Cosmos DB 組み込みロール定義 ID）

**Built-in Data ReaderのロールID:** `00000000-0000-0000-0000-000000000001`（Cosmos DB 組み込みロール定義 ID）
...
> **⚠️ 重要:** `.localConfigs`への設定だけではAzure上のBotには反映されない。Step 12「Web App アプリケーション設定」の `az webapp config appsettings set` コマンドにもSPO環境変数3つを含めること。未設定の場合、ローカルデバッグでは正常動作するがAzure上でSharePointアップロードが失敗する。
...
要件定義書に基づき、本システムでは**M365 Agents SDK**（TypeScript）を基本方針とする。Step 7「プロジェクト作成」で作成したToolkitプロジェクトには、SDK依存関係が既に含まれている。
...
az webapp config appsettings set \
  ...
  --settings \
    ...
    COSMOS_DB_ENDPOINT=https://cosmos-maintenance-poc.documents.azure.com:443/ \
    COSMOS_DB_DATABASE=maintenance-db \
    SPO_SITE_ID=<Step 10「SPOサイトID・ドライブIDの取得」で取得したサイトID> \
    SPO_DRIVE_ID=<Step 10「SPOサイトID・ドライブIDの取得」で取得したドライブID> \
    SPO_UPLOAD_FOLDER=影響候補シナリオ \
...
| `COSMOS_DB_DATABASE` | `maintenance-db` | |
| `SPO_SITE_ID` | SharePoint サイトID | Step 10「SPOサイトID・ドライブIDの取得」で取得 |
...
COSMOS_DB_ENDPOINT=https://cosmos-maintenance-poc.documents.azure.com:443/
COSMOS_DB_DATABASE=maintenance-db
SPO_SITE_ID=<SPOサイトID（Step 10「SPOサイトID・ドライブIDの取得」で取得）>
SPO_DRIVE_ID=<SPOドライブID（Step 10「SPOサイトID・ドライブIDの取得」で取得）>
SPO_UPLOAD_FOLDER=影響候補シナリオ
```
...
| Graph APIアップロード失敗（404） | `SPO_SITE_ID`または`SPO_DRIVE_ID`が誤った値 | Step 10「SPOサイトID・ドライブIDの取得」でサイトID・ドライブIDを再取得 |
| 「SharePointへのアップロードに失敗しました」 | Web Appに`SPO_SITE_ID`/`SPO_DRIVE_ID`が未設定（`.localConfigs`にのみ設定しWeb Appへの反映が漏れている） | `az webapp config appsettings list`で確認し、Step 10「SPO環境変数の設定」のWeb App反映コマンドを実行 |
