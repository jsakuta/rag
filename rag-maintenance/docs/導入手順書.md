...
本手順書は、要件定義書に基づき、Azure環境のリソース作成・設定・Bot実装・動作確認までの一連の手順を記載する。
...
| 5 | Key Vault 作成 |
| 6 | Application Insights 作成 |
| 7 | App Service (Web App) 作成 + Managed Identity有効化 |
| 8 | M365 Agents Toolkit セットアップ + Provision（Entra IDアプリ登録 + Developer Portal登録のみ） |
| 9 | Azure Bot リソース作成 + Teamsチャネル追加 |
| 10 | サービスプリンシパル作成 |
| 11 | Managed Identityロール付与 + Graph API権限付与（SPO連携用） |
| 12 | AI Search インデックス・データソース・Skillset・Indexer設定 |
| 13 | Botアプリケーション実装 + App Settings設定 + Toolkit Deploy |
| 14 | マニフェスト修正 + Teamsアプリ登録（Publish）+ 管理センター承認 |
| 15 | 動作確認（F5ローカルデバッグ + 本番テスト） |
...
Semantic RankerはBasic以上で利用可能 [17]。Free tierでは使用不可。
...
**注記:** Free プランは月1,000クエリまで [17]。超過した場合はStandardプラン（従量課金）に切り替え可能。検証規模ではFreeプランで十分。
...
| リージョン | Japan East |
| 価格レベル | Standard |
| アクセス許可モデル | Azure ロールベースのアクセス制御（RBAC） [13] |
...
![Toolkit サイドバー](screenshots/annotated/step08_1_toolkit_sidebar.png)
...
> **注意:** Microsoft 365 へのサインインはこの時点では不要。Provision実行時（Step 8「Provision実行」）に求められるため、そのタイミングで行う。
...
> **補足:** テンプレート選択時の全体構造は以下の通り。本手順ではBotアプリからAzure OpenAIを直接呼び出さない方針（要件定義書8.4参照）だが、M365 Agents SDKベースのプロジェクト構造を得るためCustom Engine Agentを選択する。LLM接続部分はStep 8「m365agents.yml のカスタマイズ」で削除・変更する。
>
...
生成されたデフォルトの`m365agents.yml`を編集し、Step 1〜7で作成済みのリソースを使用するようカスタマイズする [14]。
...
![Provision 完了](screenshots/annotated/step08_3_provision_completed.png)
...
## Step 9: Azure Bot リソース作成 + Teams チャネル追加
...
## Step 11: Managed Identity ロール付与
...
**注記:** BotアプリケーションからAzure OpenAIを直接呼び出すことはない（要件定義書8.4参照）。Embeddingの呼び出しはすべてAI Search経由（ビルトインSkill/Vectorizer [15]）で行うため、`Cognitive Services OpenAI User`はAI SearchのMIに付与する。

### Azure OpenAI ロール付与（Azureポータル）
...
### Azure AI Search ロール付与（Azureポータル） [7]
...
### Microsoft Graph API 権限付与（FR-015 SPO連携用）
...
> **⚠️ 重要:** `.localConfigs`への設定だけではAzure上のBotには反映されない。Step 13「Web App アプリケーション設定」の `az webapp config appsettings set` コマンドにもSPO環境変数3つを含めること。未設定の場合、ローカルデバッグでは正常動作するがAzure上でSharePointアップロードが失敗する。
...
- [ ] SPO環境変数を Web App にも反映した（Step 11「SPO環境変数の設定」参照）
- [ ] 各ロール付与をCLIで確認した
...
要件定義書の設計に基づき、`scenarios`と`faqs`は別コンテナであるため、データソースを2つ作成する [16]。
...
| SDK | 状態 | npmパッケージ | 備考 |
|-----|------|-------------|------|
| M365 Agents SDK | **GA**（JS, C#, Python） [2][3] | `@microsoft/agents-hosting`, `@microsoft/agents-hosting-express` | 要件定義書の基本方針。Teams以外のチャネルにも対応可能 |
...
要件定義書に基づき、本システムでは**M365 Agents SDK**（TypeScript）を基本方針とする。Step 8「プロジェクト作成」で作成したToolkitプロジェクトには、SDK依存関係が既に含まれている。
...
```bash
cd maintenance-bot
npm install @azure/cosmos @azure/identity @azure/search-documents @azure/monitor-opentelemetry
```
...
Step 8「プロジェクト作成」で生成された`src/index.ts`を編集し、まず最小限のエコーBotで動作確認する:
...
> - 接続文字列は `APPLICATIONINSIGHTS_CONNECTION_STRING` 環境変数から自動的に読み取られる（Web App には Step 6 で取得した値を設定済み）
...
az webapp config appsettings set \
  ...
  --settings \
    ...
    COSMOS_DB_ENDPOINT=https://cosmos-maintenance-poc.documents.azure.com:443/ \
    COSMOS_DB_DATABASE=maintenance-db \
    SPO_SITE_ID=<Step 11「SPOサイトID・ドライブIDの取得」で取得したサイトID> \
    SPO_DRIVE_ID=<Step 11「SPOサイトID・ドライブIDの取得」で取得したドライブID> \
    SPO_UPLOAD_FOLDER=影響候補シナリオ \
...
| `COSMOS_DB_DATABASE` | `maintenance-db` | |
| `SPO_SITE_ID` | SharePoint サイトID | Step 11「SPOサイトID・ドライブIDの取得」で取得 |
| `SPO_DRIVE_ID` | SharePoint ドライブID | Step 11「SPOサイトID・ドライブIDの取得」で取得 |
| `SPO_UPLOAD_FOLDER` | `影響候補シナリオ` | Excel出力先フォルダ名 |
| `APPLICATIONINSIGHTS_CONNECTION_STRING` | App Insights 接続文字列 | |

**注記:** Azure OpenAI 関連の環境変数は不要（Embedding は AI Search 経由で実行 [15]。要件定義書 8.4 参照）。
...
![Deploy 確認ダイアログ](screenshots/annotated/step13_1_deploy_confirm.png)

![Deploy 完了](screenshots/annotated/step13_2_deploy_completed.png)
...
Step 8「プロジェクト作成」で生成された`appPackage/manifest.json`を編集する [12]。Toolkitはプレースホルダー構文（`${{変数名}}`）を使用しており、Provision/Deploy時に自動で実際の値に置換される。
...
![Publish 完了](screenshots/annotated/step14_1_publish_completed.png)
...
![管理センター アプリ一覧](screenshots/annotated/step14_2_admin_app_list.png)

> **図の説明:** (1) アプリ一覧の検索エリア (2) 対象アプリ「maintenance-bot」

4. アプリをクリック → 詳細画面で「**公開**」ボタンをクリック

![管理センター 公開ボタン](screenshots/annotated/step14_3_admin_publish_button.png)

> **図の説明:** (1)「公開」ボタン — クリックしてアプリを公開する

5. 確認ダイアログで「**公開**」を選択

![公開確認ダイアログ](screenshots/annotated/step14_4_admin_publish_confirm.png)
...
> | 操作 | コマンド | 更新対象 | いつ必要か |
> |------|---------|---------|-----------|
> | **Deploy** | M365 Agents Toolkit: Deploy | Azure Web App 上の Bot コード（TypeScript） | ソースコード（`.ts` ファイル）を変更したとき |
> | **Publish** | M365 Agents Toolkit: Publish | Teams アプリカタログ（マニフェスト、アイコン） | `manifest.json` やアイコン画像を変更したとき |
>
...
```bash
# Cosmos DB Built-in Data Contributor ロールを付与（$DEVELOPER_OIDはStep 15「開発者向けロール付与」で取得済み）
...
```bash
# AI Search管理キーを取得（Step 15「AI Search インデックス・Indexer 更新」で取得済みの場合はスキップ可）
ADMIN_KEY=$(az search admin-key show \
  --service-name srch-maintenance-poc \
  --resource-group rg-maintenance-poc \
  --query primaryKey -o tsv)
...
![F5 アカウント選択](screenshots/annotated/step15_1_f5_account_select.png)

![F5 アプリ詳細](screenshots/annotated/step15_2_f5_app_detail.png)
...
COSMOS_DB_ENDPOINT=https://cosmos-maintenance-poc.documents.azure.com:443/
COSMOS_DB_DATABASE=maintenance-db
SPO_SITE_ID=<SPOサイトID（Step 11「SPOサイトID・ドライブIDの取得」で取得）>
SPO_DRIVE_ID=<SPOドライブID（Step 11「SPOサイトID・ドライブIDの取得」で取得）>
SPO_UPLOAD_FOLDER=影響候補シナリオ
```
...
![Teams Web トップ](screenshots/annotated/step15_3_teams_web_top.png)

2. 左サイドバーの「アプリ」→「アプリストア」を開く

![アプリストア](screenshots/annotated/step15_4_apps_store.png)

> **図の説明:** (1) 左サイドバーの「アプリ」アイコン (2)「組織向けに開発」カテゴリ

3. 「**組織向けに開発**」セクションでアプリを確認

![組織向けアプリ](screenshots/annotated/step15_5_org_apps.png)

> **図の説明:** (1) 対象アプリ「maintenance-botdev」 (2)「追加」ボタン

4. アプリをクリック →「**追加**」ボタンを押す

![アプリ詳細](screenshots/annotated/step15_6_app_detail.png)

> **図の説明:** (1)「追加」ボタン — クリックしてアプリをインストールする

5. 追加完了 → Bot との 1:1 チャットが開く

![Bot チャットオープン](screenshots/annotated/step15_7_bot_chat_open.png)

6. メッセージを送信 → Adaptive Card で検索カードが応答されることを確認

![Bot 応答（Adaptive Card）](screenshots/annotated/step15_8_bot_response.png)

> **図の説明:** (1) カードタイトル「事務改定 影響候補検出」 (2) タブ切替（シナリオ/FAQ） (3) カテゴリ選択チェックボックス (4) 検索モードボタン（意味検索/キーワード検索）
...
| 症状 | 原因の可能性 | 対処法 |
|------|------------|--------|
| `Invalid URL`でサーバーが起動しない | Azure SDKクライアントがモジュールトップレベルで初期化されている [3] | `SearchClient`/`CosmosClient`を遅延初期化パターンに変更（Step 13の注記参照） |
| `Invalid URL`でサーバーが起動しない | `.localConfigs`の`AI_SEARCH_ENDPOINT`等が空 | `.localConfigs`の必須項目を確認・設定（Step 15「F5ローカルデバッグ」参照） |
| F5デバッグで認証エラー | `.localConfigs`の`clientId`/`clientSecret`が空 | `env/.env.dev`からBOT_IDを転記、シークレットは`az ad app credential reset`で再生成（Step 15「F5ローカルデバッグ」参照） |
...
| 403 Forbidden（F5ローカル） | 開発者アカウントにAI Searchのロールが未付与 | Step 15「開発者向けロール付与」のAI Search データプレーンロール付与を実施（反映に数分かかる） |
...
| Adaptive Cardが表示されない | カードサイズ25KB超過 [18] | 候補件数を確認し、ページネーションフォールバックを検討 |
| Graph APIアップロード失敗（403） | Web AppのMIに`Files.ReadWrite.All`が未付与 | Step 11「Microsoft Graph API 権限付与」を実施 |
| Graph APIアップロード失敗（404） | `SPO_SITE_ID`または`SPO_DRIVE_ID`が誤った値 | Step 11「SPOサイトID・ドライブIDの取得」でサイトID・ドライブIDを再取得 |
| 「SharePointへのアップロードに失敗しました」 | Web Appに`SPO_SITE_ID`/`SPO_DRIVE_ID`が未設定（`.localConfigs`にのみ設定しWeb Appへの反映が漏れている） | `az webapp config appsettings list`で確認し、Step 11「SPO環境変数の設定」のWeb App反映コマンドを実行 |
| Application Insightsにログが表示されない | `@azure/monitor-opentelemetry`が未インストール、または`useAzureMonitor()`が未呼出 | Step 13「追加パッケージのインストール」と「Application Insights の有効化」を確認し、再デプロイ |
...
| `clientId` 設定済みだが認証失敗 | `MicrosoftAppId` のみ設定し `clientId` が未設定（またはその逆） [3] | M365 Agents SDK は `clientId`/`clientSecret`/`tenantId` を読む。`MicrosoftApp*` と**両方設定**が必要（Step 13 参照） |
