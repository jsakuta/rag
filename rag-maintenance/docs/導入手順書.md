...
> **事前確認:** `scripts/index-definition.json` と `scripts/skillset.json` には Azure OpenAI の `resourceUri` / `deploymentId` / `modelName`、`scripts/datasource-*.json` には Cosmos DB の `ResourceId` とデータベース名が埋め込まれている。`poc` 以外の環境では、実行前に対象環境の値へ置換すること。
