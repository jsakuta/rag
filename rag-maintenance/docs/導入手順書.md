...
| 8 | Log Analytics Workspace | — | Application Insightsのバックエンド（Step 5でApplication Insights作成時に自動作成） |
...
- [ ] リソースグループ `rg-maintenance-poc` が正常に作成された
...
- [ ] Application Insights リソースが正常に作成された
- [ ] 接続文字列を記録した（Step 12 のアプリケーション設定で使用）
...
> **引き継ぎコードがある場合:** 開発済みの `maintenance-bot/` フォルダを受領している場合、「プロジェクト作成」「m365agents.yml のカスタマイズ」はスキップする。VS Code で `maintenance-bot/` フォルダを直接開き、「環境変数ファイルの設定」から開始すること。`m365agents.yml` および `.vscode/`（F5デバッグ設定）はカスタマイズ済みで同梱されている。`env/` ディレクトリは空の状態で同梱されているので、下記「環境変数ファイルの設定」に従い `env/.env.dev` を新規作成し、`AZURE_SUBSCRIPTION_ID` 等を新環境の値に設定する。
...
![図：Toolkit サイドバー](screenshots/annotated/step07_1_toolkit_sidebar.png)
...
![図：プロジェクト作成完了](screenshots/annotated/step07_2_project_created.png)
...
**完了後:** `env/.env.dev`に`BOT_ID`と`TEAMS_APP_TENANT_ID`（テナントID）が、`env/.env.dev.user`に`SECRET_BOT_PASSWORD`が書き込まれる

![図：Provision 実行](screenshots/annotated/step07_3_provision_completed.png)
...
- [ ] `TEAMS_APP_TENANT_ID`（テナントID）が `env/.env.dev` に記録された（手動確認: `az account show --query tenantId -o tsv`）
...
| Cosmos DB | `Cosmos DB Account Reader Role` | **AI Search**のManaged Identity | Indexerのデータソース接続メタデータ読み取り（管理プレーン） |
...
取得したドライブIDを Bot プロジェクトのローカル環境変数ファイル（`maintenance-bot/.localConfigs`、Step 14 で詳述）に記録する。`SPO_SITE_ID`は現行コードでは参照していないため、運用メモとして保持する場合のみ設定する:
...
> **注意:** Web App への環境変数反映は **Step 12** で他のアプリケーション設定と一括で行う。`.localConfigs` への設定だけでは Azure 上の Bot には反映されない。
...
- [ ] SPOドライブIDを取得し、`.localConfigs`に記録した（Web App への反映は Step 12 で実施）
...
| `COSMOS_DB_DATABASE` | `maintenance-db` | |
| `SPO_DRIVE_ID` | SharePoint ドライブID | Step 10「SPOドライブIDの取得」で取得 |
...
![図：Deploy 確認ダイアログ](screenshots/annotated/step12_1_deploy_confirm.png)

![図：Deploy 完了](screenshots/annotated/step12_2_deploy_completed.png)
...
> **将来の拡張に関する注記:** 要件定義書 NFR-001 では、Action.Executeタイムアウト超過時に「処理中」カードを即時返却し、バックグラウンド処理後にproactiveActivityで結果通知する構成を記載している。proactiveActivityの実装にはConversation Referenceの保存が必要となるため、将来的にCosmos DBへの追加コンテナや環境設定が必要になる可能性がある。詳細は実装ガイドで扱う。
...
![図：Publish 完了](screenshots/annotated/step13_1_publish_completed.png)
...
![図：管理センター アプリ一覧](screenshots/annotated/step13_2_admin_app_list.png)

> **図の説明:** (1) アプリ一覧の検索エリア (2) 対象アプリ「maintenance-botdev」

4. アプリをクリック → 詳細画面で「**公開**」ボタンをクリック

![図：管理センター 公開ボタン](screenshots/annotated/step13_3_admin_publish_button.png)

> **図の説明:** (1)「公開」ボタン — クリックしてアプリを公開する

5. 確認ダイアログで「**公開**」を選択

![図：公開確認ダイアログ](screenshots/annotated/step13_4_admin_publish_confirm.png)
...
![図：F5 アカウント選択](screenshots/annotated/step14_1_f5_account_select.png)

![図：F5 アプリ詳細](screenshots/annotated/step14_2_f5_app_detail.png)
...
![図：Teams Web トップ](screenshots/annotated/step14_3_teams_web_top.png)

2. 左サイドバーの「アプリ」→「アプリストア」を開く

![図：アプリストア](screenshots/annotated/step14_4_apps_store.png)

> **図の説明:** (1) 左サイドバーの「アプリ」アイコン (2)「組織向けに開発」カテゴリ

3. 「**組織向けに開発**」セクションでアプリを確認

![図：組織向けアプリ](screenshots/annotated/step14_5_org_apps.png)

> **図の説明:** (1) 対象アプリ「maintenance-botdev」 (2)「追加」ボタン

4. アプリをクリック →「**追加**」ボタンを押す

![図：アプリ詳細](screenshots/annotated/step14_6_app_detail.png)

> **図の説明:** (1)「追加」ボタン — クリックしてアプリをインストールする

5. 追加完了 → Bot との 1:1 チャットが開く

![図：Bot チャットオープン](screenshots/annotated/step14_7_bot_chat_open.png)

6. メッセージを送信 → Adaptive Card で検索カードが応答されることを確認

![図：Bot 応答（Adaptive Card）](screenshots/annotated/step14_8_bot_response.png)

> **図の説明:** (1) カテゴリ単一選択プルダウン (2) 各分野の表示件数プルダウン (3) 検索モードボタン（意味検索/キーワード検索）

カテゴリ選択肢と表示件数の詳細は、プルダウン展開状態のキャプチャで確認する。

![図：カテゴリ選択（展開）](screenshots/step14_8_category_select_open.png)

> **図の説明:** シナリオカテゴリの単一選択プルダウンを展開した状態。選択肢はスマイル/相続/内部事務/取引時確認。

![図：表示件数選択（展開）](screenshots/step14_8_count_select_open.png)

> **図の説明:** 各分野の表示件数プルダウンを展開した状態。選択肢は10件刻み、最大100件。

シナリオタブで要修正フラグを保存すると、完了カードが表示される。

![図：要修正保存完了](screenshots/step14_9_needs_update_complete.png)
...
![図：Excel 出力完了](screenshots/step14_10_excel_export_complete.png)
