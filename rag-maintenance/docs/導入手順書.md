...
Step 8: M365 Agents Toolkit セットアップ + Provision
  ...
  ↓
Step 9: Managed Identityロール付与 + Graph API権限付与（SPO連携用）
  ↓
...
### 11.6 Microsoft Graph API 権限付与（FR-015 SPO連携用）
...
#### 11.6.2 Graph API アプリケーション権限の付与
...
SPO_UPLOAD_FOLDER=ImpactReports
```
...
### 15.0 前提準備（テストデータ投入・権限付与・Indexer実行）

動作確認には、Cosmos DBにテストデータを投入し、AI Searchインデックスに同期する必要がある。

#### 15.0.1 開発者向けロール付与（AI Search + Cosmos DB）

F5ローカルデバッグでは`DefaultAzureCredential`が開発者のAzure CLIログインアカウントを使用する。Step 9で付与したManaged Identityのロールとは別に、開発者個人のアカウントにもロール付与が必要。
...
```bash
# Cosmos DB Built-in Data Contributor ロールを付与（$DEVELOPER_OIDは15.0.1で取得済み）
...
**注記:** このロールはデータプレーン専用であり、Azure Portalの「アクセス制御(IAM)」タブには表示されない。`az cosmosdb sql role assignment list`コマンドで確認する。Web AppのManaged Identity（Step 9で付与済み）とは別に、開発者個人のアカウントにも付与が必要。

#### 15.0.2 テストデータ準備（Excel→JSON変換）

rag-gemini プロジェクトの実データ（Excel）を Cosmos DB 投入用 JSON に変換する。
...
#### 15.0.4 AI Search インデックス・Indexer 更新
...
COSMOS_DB_ENDPOINT=https://cosmos-maintenance-poc.documents.azure.com:443/
COSMOS_DB_DATABASE=maintenance-db
SPO_SITE_ID=<SPOサイトID（Step 9 11.6.1で取得）>
SPO_DRIVE_ID=<SPOドライブID（Step 9 11.6.1で取得）>
SPO_UPLOAD_FOLDER=ImpactReports
```
...
**注意:** シークレットを再生成すると、既存のシークレットは無効化される。Azure Web Appの環境変数`SECRET_BOT_PASSWORD`も同じ値に更新すること。
...
| 症状 | 原因の可能性 | 対処法 |
|------|------------|--------|
| `Invalid URL`でサーバーが起動しない | Azure SDKクライアントがモジュールトップレベルで初期化されている | `SearchClient`/`CosmosClient`を遅延初期化パターンに変更（Step 11の注記参照） |
| `Invalid URL`でサーバーが起動しない | `.localConfigs`の`AI_SEARCH_ENDPOINT`等が空 | `.localConfigs`の必須項目を確認・設定（15.1参照） |
| F5デバッグで認証エラー | `.localConfigs`の`clientId`/`clientSecret`が空 | `env/.env.dev`からBOT_IDを転記、シークレットは`az ad app credential reset`で再生成（15.1参照） |
...
| 403 Forbidden（F5ローカル） | 開発者アカウントにAI Searchのロールが未付与 | 15.0.1の「AI Search データプレーン」ロール付与を実施（反映に数分かかる） |
| 403 Forbidden（Azure Web App） | Web AppのManaged Identityのロール付与不足 | Step 9のロール付与を再確認 |
...
| Graph APIアップロード失敗（403） | Web AppのMIに`Files.ReadWrite.All`が未付与 | Step 9 11.6.2のGraph API権限付与を実施 |
| Graph APIアップロード失敗（404） | `SPO_SITE_ID`または`SPO_DRIVE_ID`が誤った値 | Step 9 11.6.1でサイトID・ドライブIDを再取得 |
