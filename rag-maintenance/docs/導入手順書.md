...
**注記:** Free プランは月1,000クエリまで [17]。超過した場合はStandardプラン（従量課金）に切り替え可能。PoCの検証規模ではFreeプランで十分。
...
### 13.3 Azure AI Search ロール付与（Azureポータル） [7]
...
### 13.4 Key Vault ロール付与（Azureポータル） [13]
...
### 13.5 Cosmos DB ロール付与（Azure CLI必須） [8]
...
> **⚠️ 重要:** `.localConfigs`への設定だけではAzure上のBotには反映されない。Step 13（15.4節）の `az webapp config appsettings set` コマンドにもSPO環境変数3つを含めること。未設定の場合、ローカルデバッグでは正常動作するがAzure上でSharePointアップロードが失敗する。
...
- [ ] SPO環境変数を Web App にも反映した（13.6.3 参照）
- [ ] 各ロール付与をCLIで確認した
...
### 15.3.1 Application Insights の有効化
...
Step 10.3で生成された`appPackage/manifest.json`を編集する [12]。Toolkitはプレースホルダー構文（`${{変数名}}`）を使用しており、Provision/Deploy時に自動で実際の値に置換される。
...
| 「SharePointへのアップロードに失敗しました」 | Web Appに`SPO_SITE_ID`/`SPO_DRIVE_ID`が未設定（`.localConfigs`にのみ設定しWeb Appへの反映が漏れている） | `az webapp config appsettings list`で確認し、Step 11 13.6.3のWeb App反映コマンドを実行 |
| Application Insightsにログが表示されない | `@azure/monitor-opentelemetry`が未インストール、または`useAzureMonitor()`が未呼出 | Step 13 15.2のパッケージ追加と15.3.1の初期化コードを確認し、再デプロイ |
