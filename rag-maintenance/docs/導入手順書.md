...
プロジェクトに同梱のスクリプト `scripts/seed-cosmos.js` を使用する。
...
| コンテナ | 件数 | 内容例 |
|---------|------|--------|
| `scenarios` | 5件 | 口座開設手続き、海外送金手続き、定期預金解約、住宅ローン申込、キャッシュカード再発行 |
| `faqs` | 10件 | 口座開設FAQ、通帳紛失、海外送金手数料、送金身分証明、住宅ローン書類 他 |

**注記:** スクリプトは `upsert` を使用するため、複数回実行しても安全（既存データを上書き）。本番データを投入する場合は、CSV/JSON形式でデータを準備し、スクリプトを改修する。

#### 15.0.3 Indexer手動実行

テストデータ投入後、AI Search Indexerを手動実行してインデックスに同期する。Indexerのスケジュール実行（1時間ごと）を待たずに即時反映するための操作。
...
HTTP 202が返れば実行開始。Indexer完了後（通常1〜2分）に検索が可能になる。

**確認方法:**

```bash
# Indexer状態を確認（statusが"success"であればOK）
