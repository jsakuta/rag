...
**注記:** `maintenance-bot/infra/azure.bicep` がこの repo で直接作成するのは Managed Identity、App Service Plan、Azure Web App である。Azure AI Search、Azure OpenAI、Cosmos DB、Application Insights、Azure Bot Service は既存環境または別手順で用意する前提とする。Key Vault はPoC段階では未使用。本番移行時に導入を検討する。
...
| combinedContent | Edm.String | ✓（ja.microsoft） | — | — | title + content（結合テキスト）。Skillsetのベクトル化入力ソース（このフィールドからcontentVectorを生成） |
...
| Cosmos DB書き込み認証 | Web AppのManaged Identityに`Cosmos DB Built-in Data Contributor`ロールを付与（FR-007 FAQ削除・FR-008 判定記録で使用） |
