...
Phase 1（ローカル検証）では Vertex AI（Gemini embedding-001）のみで開発・検証していたが、Phase 2（クラウド実装）は B&DX Azure 上で実装予定のため、Azure OpenAI（text-embedding-3-large）の方が実装上容易である。2つのモデルは埋め込みベクトルの性質が異なるため、ローカルで Gemini のみ検証して Phase 2 で Azure に切り替えた際に精度が低下するリスクがあった。

このリスクを最小化するため、**ローカル段階から両モデルで並行検証し、精度差がないことを確認する**方針を採用した（2026/01/26 事務企画部定例で決定）。検証の結果、両モデルとも同等の精度が確認され、インフラとの整合性から Azure を採用する方向で合意している（2026/02/17 事務企画部定例）。
